...
self.location='?p=labdaset&q=' + val;
}
</script>
...
on a.id=b.kwitansi_id order by kode_satker, tgl_kwitansi desc");
...
while (($row1 = oci_fetch_array($sql, OCI_BOTH)) != false) {
 ...
 echo     "
  <tr class='gradeX'>
    ...
    <td width='130px'>". $row1["TGL_KWITANSI"] . "</td>
...
else{  $sql  = oci_parse($conn, "select * from (select id,kode_satker,tgl_kwitansi,no_kwitansi,kode_akun,keterangan
from sakti_app.ben_t_kwitansi_header
