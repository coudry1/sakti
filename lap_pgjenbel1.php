...
{
var val=form.pfa.options[form.pfa.options.selectedIndex].value;
self.location='?p=lpjb&q=' + val;
}
</script>
...
<div align='center'>
<form action="?p=pfa" method="post">
<select name="pfa" id="pfa" onChange="reload(this.form)">
...
$sql  = oci_parse($conn, "select c.kdsatker, c.deskripsi, c.akun, d.jenis_belanja, c.pagu, c.sisa_pagu from (
select a.kdsatker, b.deskripsi, a.akun, a.pagu, a.realisasi_kom, a.realisasi_ben, a.realisasi_spm, a.realisasi_glp, a.sisa_pagu from (
select substr(kode_coa,1,6) kdsatker, substr(kode_coa,12,6) akun, pagu, realisasi_kom, realisasi_ben, realisasi_spm, realisasi_glp, sisa_pagu  from  SAKTI_APP.GLP_T_FA_HEADER 
where substr(kode_coa,1,6)='$kdsatker' group by substr(kode_coa,1,6), kode_coa, pagu, realisasi_kom, realisasi_ben, realisasi_spm, realisasi_glp, sisa_pagu 
order by substr(kode_coa,1,6)) a left join (select kode, deskripsi from SAKTI_APP.ADM_R_SATKER) b on a.kdsatker=b.kode
) c left join (select kode, deskripsi jenis_belanja from SAKTI_APP.ADM_R_AKUN) d on c.akun=d.kode order by kdsatker, akun");
...
<tr><td colspan='6' align='center'><strong>PAGU PER JENIS BELANJA</strong></td></tr>
...
<table height='50px' width='1150px' align='center' border='1'>
  <tr>
    ...
    <th class='head1' align='center'> Akun </td>
    <th class='head1' > Jenis Belanja </td>
...
$sisa_pagu=number_format($row1["SISA_PAGU"],0,",",".");
 echo     "
  <tr class='gradeX'>
    <td>$no.</td>
    <td align='center' width='70px'>". $row1["KDSATKER"] ."</td>
  	<td width='70px' align='center'>". $row1["AKUN"] . "</td>
	<td width='500px'>". $row1["JENIS_BELANJA"] . "</td>
