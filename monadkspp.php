...
$sql  = oci_parse($conn, "select count(*) as JUMLAH_SPP FROM SAKTI_APP.SPM_T_SPP where to_char(modified_date, 'yyyy-mm-dd') = to_char(sysdate, 'yyyy-mm-dd')");
oci_define_by_name($sql,'JUMLAH_SPP',$jumlah_spp);
oci_execute($sql);
oci_fetch($sql);
...
<table width='1470' border='1' align='center'>
   <caption><div align='center'> ADK SPP </div></caption> 
<tbody>
    <tr><th>Nama Satker</th><th>Jenis Tagihan</th><th width='600px'>Uraian</th><th>Nilai</th><th>Tanggal SPP</th><th>Stage</th></tr>
	  <tr height='25px'>
    <td></td>
    <td></td>
    <td></td>
...
SELECT  kd_satker, kd_jns_spp, created_date, uraian, jml_pengeluaran nilai_rupiah, tgl_spp, sts_data  FROM SAKTI_APP.SPM_T_SPP where to_char(modified_date, 'yyyy-mm-dd') = to_char(sysdate, 'yyyy-mm-dd') and sts_data='SPM_STS_DATA_05') a left join
